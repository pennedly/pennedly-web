...
// Pattern Study — paste the TEXT of posts you admire, get back the
// reusable techniques. Hard stance, surfaced in the UI: paste text,
// not links/profiles. Pennedly never opens links or reads other
// accounts. The backend rejects URLs/handles too (defense in depth).
//
// Each returned pattern has a "suggested_do_rule" the user can fold
// into their own role_book do_list in one click (reuses the
// apply-fix add_item path).
  ...
  async function onAnalyze() {
    ...
    try {
      ...
      if (e instanceof ApiError) {
        // Backend returns { reason, message } for validation failures.
