...
// ── real brand marks ──────────────────────────────────────────────────────────
export function ThreadsLogo({ s = 22 }: { s?: number }) {
  return (
...
export function LinkedInLogo({ s = 22 }: { s?: number }) {
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
export function NetLogo({ network, s = 10 }: { network: string; s?: number }) {
  if (network === "threads") return <ThreadsLogo s={s} />;
  if (network === "linkedin") return <LinkedInLogo s={s} />;
  return null;
}
