...
export function IdleView({ postsAnalyzed, onRun }: { postsAnalyzed: number; onRun: () => void }) {
  const { t } = useTranslation();
  return (
    <div className="flex flex-col items-center rounded-2xl border border-border bg-surface px-10 pb-9 pt-10 text-center shadow-sm">
      ...
      <div className="mt-[18px] flex items-center gap-2.5 text-caption text-text-subtle">
...
export function PatternCard({ p }: { p: PatternCardModel }) {
  ...
  return (
    <article className="rounded-lg border border-border bg-surface px-[22px] pb-[18px] pt-5 shadow-sm transition-colors hover:border-text/12" style={{ animation: "card-in var(--duration-slow) var(--ease-entrance) both" }}>
      ...
      <div className="flex items-center gap-[18px] max-[600px]:gap-3.5">
        <span className="shrink-0 text-display font-semibold tabular-nums leading-[0.9] tracking-[-0.02em] text-accent max-[600px]:text-h1">{p.stat}</span>
        <span className="text-h3 font-semibold leading-[1.32] tracking-[-0.006em]">{p.headline}</span>
      </div>
...
function EvRow({ ev, max, lead }: { ev: { label: string; display: string; value: number; n: number }; max: number; lead?: boolean }) {
  return (
    <div className="grid grid-cols-[168px_1fr_auto] items-center gap-3.5 max-[600px]:grid-cols-[1fr_auto]">
      <span className={cn("truncate text-small max-[600px]:order-1", lead ? "text-text" : "text-text-muted")}>{ev.label}</span>
      <div className="h-[22px] overflow-hidden rounded-sm border border-border bg-surface-2 max-[600px]:order-3 max-[600px]:col-span-2">
      ...
      <span className={cn("whitespace-nowrap text-small font-semibold tabular-nums max-[600px]:order-2", lead ? "text-text" : "text-text-muted")}>
