...
(function () {
  ...
  function layer3Body() {
    return '<div class="rce-stub"><span class="rs-ico">' + ic('shieldhouse', 16) + '</span><div>'
      ...
      + '<div class="rs-d">Свой лимит ответов, частота проверки и тихие часы — вместо общих. Сейчас всё наследуется из «Правил дома»; точечное переопределение появится во второй волне.</div>'
  ...
  function buildEditor(cfg) {
    ...
    left = '<div class="rce-main">'
      ...
      + '<div class="rce-layers">'
        ...
        + layer('3', 'Только для этого сценария', true, 'Переопределить «Правила дома» здесь', layer3Body(), cfg.layer3)
        + '</div>'
      + foot(cfg) + '</div>';
    return '<div class="rce">' + topbar(name, cfg.isNew) + '<div class="rce-grid">' + left + aside(cfg) + '</div>' + (cfg.modal ? bigTextModal(cfg.modal) : '') + '</div>';
  }

  window.RecipeEditor = { build: buildEditor, card: recipeCard, frame: frame, ic: ic };

  /* ── auto-mount any [data-recipe] hosts ── */
  function mountAll() {
    var hosts = document.querySelectorAll('[data-recipe]');
    for (var i = 0; i < hosts.length; i++) {
      var el = hosts[i];
      var cfg = {};
      try { cfg = JSON.parse(el.getAttribute('data-recipe') || '{}'); } catch (e) { cfg = {}; }
      el.innerHTML = frame(buildEditor(cfg), cfg.dark);
    }
    var cards = document.querySelectorAll('[data-recipe-card]');
    for (var j = 0; j < cards.length; j++) {
      var ce = cards[j], ccfg = {};
      try { ccfg = JSON.parse(ce.getAttribute('data-recipe-card') || '{}'); } catch (e2) { ccfg = {}; }
