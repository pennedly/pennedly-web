...
(function () {
  ...
  function sidebar(lang, opts) {
    ...
    function row(id, icon, label, badge) {
      var cls = "acc-sb-row" + (active === id ? " acc-sb-row--active" : "");
      var b = badge ? "<span class='acc-sb-badge'>" + badge + "</span>" : "";
      return "<a class='" + cls + "'" + (active === id ? " aria-current='page'" : "") + ">" + ic(icon, 17) + "<span class='acc-sb-rowtxt'>" + label + "</span>" + b + "</a>";
    }
    var rows = row("dashboard", "grid", t.navDashboard);
    if (multi) rows += row("brands", "layers", t.navBrands, opts.brandsCount);
    rows += row("advisor", "advisor", t.navAdvisor);
    rows += row("settings", "settings", t.navSettings);
