...
> **📋 АУДИТ 2026-06-02 (4 агента, все 16 экранов).** Ключевой вывод: **общий shell + design-system уже стоят на всех экранах** — осталась не верстка с нуля, а **поведение + подключение полей бэка** (Фаза 1 уже их отдаёт, фронт их ещё не читает). **Готово 16/16 экранов — Фаза 2 функционально закрыта** (Explore, Landing, Legal, Audits, Patterns, Onboarding, Mentions, Autopilot, Login, Feed, Settings, Style rules, Stats, Replies, Studio, **Voice — включая Q8 перевод**). Осталось: кросс-экранный **Q24** optimistic+Undo (бэк-блок) · Фаза 3 (Meta). Подключённые поля: `avg_reposts`(Q64✅) · `min_posts_to_analyze`(Q53✅) · `category`+`old_text/new_text`(Q51/Q75✅) · `SelfStudyExample.metric`(Q56✅) · `GET /study/latest`(Q54✅) · Patterns ×N+Topics-chip(Q42/Q55✅). Осталось подтянуть: `auto_replied`(Q3, Replies) · `posts_analyzed`(Q67, Voice) · **`RoleBookSections` flat→typed objects(Q60, Voice — полный редактор)**.
>
...
| 2 | **Studio** | ✅ done* | Q62 reply-черновики read-only ✅ · Q14 voice-pill ✅ · Q25 ✅ — одна primary + `CardMenu` ⋯-overflow [tweak/edit/reject/delete], hard-delete в меню + inline-confirm, `focus-within:z` чтобы меню перекрывало след. карту. (Q9 ✅). *Q24 (optimistic+Undo) — кросс-экранный паттерн, ОТЛОЖЕН: бэк не имеет un-reject/restore-draft эндпоинта (approve требует `pending`), нужен бэкенд + единый Toast-undo на 4 экранах |
