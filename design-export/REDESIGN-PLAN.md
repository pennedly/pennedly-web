...
| 10 | **Voice/role-book** | 🟢 ✅* | **Q60 ✅ — полный типизированный редактор всех 5 секций**: `normalizeSections` коэрсит объекты бэка (и легаси-строки) в типы; `ThemesSection` {label,note}, `TraitsSection` {label?,text}, `RulesSection` {text}, `ExamplesSection` {context,text} (Q16); стабильные `id` round-trip; флаг по `id` (`ConflictItem.id`, не по тексту). Бэк-лор закрыт: `add_item` восстановлен (Explore «Add to Voice») + `ConflictItem.id` в ответе lint. · Q67 ✅ hero · Q23 ✅ empty. **\*Осталось только Q8** — перевод по секциям (globe-тоггл + read-only translated view через `/api/translate`) |
