...
> **📋 АУДИТ 2026-06-02 (4 агента, все 16 экранов).** Ключевой вывод: **общий shell + design-system уже стоят на всех экранах** — осталась не верстка с нуля, а **поведение + подключение полей бэка** (Фаза 1 уже их отдаёт, фронт их ещё не читает). **Готово 16/16 экранов — Фаза 2 функционально закрыта** (Explore, Landing, Legal, Audits, Patterns, Onboarding, Mentions, Autopilot, Login, Feed, Settings, Style rules, Stats, Replies, Studio, **Voice — включая Q8 перевод**). **Q24** (optimistic+Undo) ✅ — закрыт **без бэкенда** через deferred-commit (общий хук `useDeferredCommit`): действие применяется в UI сразу, реальный запрос откладывается на 5с, тост «Отменить» его отменяет. На 4 экранах (Studio reject+delete · Replies skip→restore · Autopilot/Style-rules delete-rule). Осталось: **Фаза 3 (Meta)**. Подключённые поля: `avg_reposts`(Q64✅) · `min_posts_to_analyze`(Q53✅) · `category`+`old_text/new_text`(Q51/Q75✅) · `SelfStudyExample.metric`(Q56✅) · `GET /study/latest`(Q54✅) · Patterns ×N+Topics-chip(Q42/Q55✅). Осталось подтянуть: `auto_replied`(Q3, Replies) · `posts_analyzed`(Q67, Voice) · **`RoleBookSections` flat→typed objects(Q60, Voice — полный редактор)**.
>
...
| 2 | **Studio** | ✅ done | Q62 reply-черновики read-only ✅ · Q14 voice-pill ✅ · Q25 ✅ — одна primary + `CardMenu` ⋯-overflow [tweak/edit/reject/delete], hard-delete в меню + inline-confirm, `focus-within:z` чтобы меню перекрывало след. карту. **Q24 ✅** — reject/delete оптимистично убирают карту + тост «Отменить» (deferred-commit, без бэкенда: реальный `rejectDraft`/`deleteDraft` уходит через 5с, Undo отменяет). (Q9 ✅) |
...
| 4 | **Replies** | ✅ done | Q18 ✅ — master-detail: слева `PostMaster` (посты + «N to answer»), справа `PostContext`-шапка + per-post статус-фильтр + комменты выбранного поста; PostRail и per-card инсет убраны; грузим все комменты, фильтр клиентский. Q77 ✅ — статус-бакеты (`approved`→Draft); account-wide `status_counts` → topbar-пилюля. Q3 ✅ бейдж auto_replied · Q10 ✅ перевод ответа · (Q37 empty ✅) |
...
| 6 | **Stats** | ✅ done | Q19 ✅ — 6 периодов (today/yesterday/7d/30d/90d/all), `fetchStats({period})`, бэк уже отдаёт period+per-post `label` · Q39 ✅ — пунктирная линия среднего за период + окраска баров выше(success)/ниже · Q12 ✅ — убран posts/week-чарт (один чарт avg views/post) · Q80 ✅ — «Updated hourly» pill (clock). (Q47 тиры ✅) |
...
| 10 | **Voice/role-book** | ✅ done | **Q60 ✅** типизированный редактор всех 5 секций (`normalizeSections`; `ThemesSection` {label,note}, `TraitsSection` {label?,text}, `RulesSection` {text}, `ExamplesSection` {context,text}; стабильные `id` round-trip; флаг по `ConflictItem.id`). **Q8 ✅** — режим перевода: globe-меню (8 локалей) под заголовком → весь голос (intro + поля каждого пункта) переводится по-элементно через `/api/translate` (пул 6, дедуп, бэк-кэш, race-токен), read-only + баннер «view original»; check/re-extract/правка отключены. · Q67 ✅ hero · Q23 ✅ empty · Q16 ✅ примеры. Бэк-лор закрыт: `add_item` восстановлен (Explore) + `ConflictItem.id` |
| 11 | **Style rules** | ✅ done | Q29 ✅ — Your rules первыми (flex `order`), intro_lead переписан · Q30 ✅ — встроенные в всегда-видимых группах по категориям + счётчики (chip-фильтр убран) · Q44 ✅ — live before→after для `human_punctuation` (— → -, «» → ", … → ...) · Q49 ✅ — 14 встроенных правил title+desc через i18n по `key` (`RULE_I18N`, рус. `body` остаётся промптом) · Q81 ✅ — toast по виду (post/reply/both) + «both» в пикере (дефолт). (Q11 ✅) |
