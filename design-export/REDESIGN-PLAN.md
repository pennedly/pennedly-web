...
| 10 | **Voice/role-book** | 🟡 L | Q60 интерим-фикс ✅ (`flattenSections`) · Q67 ✅ — hero «Analyzed N posts · Updated <date>» из `posts_analyzed`+`activated_at` (v<id>/parent убраны) · Q23 ✅ — GET 404 → `EmptyVoice` с «Extract my voice». Q16 ✅ — примеры типизированы (`RoleBookExample {id,context,text}`), edit-режим Post/Reply `<select>`+textarea, read-режим context-чип; `flattenSections` сохраняет examples объектами (норм. context→post/reply). Осталось (полный типизированный редактор остальных 5 секций): **Q60 (правка объектов с вторичными полями note/text + стабильные ids)** · Q8 перевод по секциям |
