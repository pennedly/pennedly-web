...
| `Voice-SPEC.html` | `archive/Voice-SPEC.html` *(см. правки 26 июля)* |
| `Style-Rules-Mobile-SPEC.html` | `screens/style-rules/Style-Rules-Mobile-SPEC.html` |
| `Style-Rules-SPEC.html` | `archive/Style-Rules-SPEC.html` *(см. правки 26 июля)* |
...
| `design-export/PennedlyDesign/Voice-Tabs-SPEC.html` | `screens/role-book/Voice-Tabs-SPEC.html` *(см. правки 26 июля)* |
...
`role-book` (Голос) описан десктопной спекой `Voice-Tabs-SPEC.html` — мобильной версии этого экрана в проекте нет.
...
## Правки 26 июля (три штуки)

### 1. Feed-SPEC: панель роста снята, кнопка догрузки описана

Панель роста поста удалена из продукта 10 июля, а спека её всё ещё описывала. Вычищено полностью — **0 упоминаний** `.trend-panel`, `TrendChart`, `.tc-t`/`.tc-s`, `.chart-wrap`/`.chart-axis` и кнопки «Growth»:

- оглавление: «.metrics + .trend-panel» → «.metrics»;
- §2.8 переименован в «Метрики .metrics», четыре строки анатомии (контейнер, подписи, TrendChart+SVG, ось) вырезаны;
- из футера карточки убрана кнопка «Growth» (IcChart + IcChevDown 14); остальные элементы футера не тронуты;
- §5.1: удалены строки `.trend-panel` и `TrendChart SVG`;
- §4.4 «Карточка» — убран пункт статуса «growth свёрнут/развёрнут» и панель «Growth (график роста)» из живого разбора состояний;
- попутно сняты остаточные упоминания в §3 (адаптив), §4.7 (состояния), §4.8 (анимации: card-in и chevron rotate), §5.2 (типографика: tc-t/tc-s), §5.3 (токен surface-2), твиках и чек-листе.

Добавлена **кнопка «Показать старые посты»** — новый §2.10, плюс строка в §5.1. Пронумерованные ниже подразделы сдвинулись: EmptyState → 2.11, ConfirmDelete → 2.12.

### 2. Overview-Cockpit: секция growth-аудита снята

- §5 «Строка growth-аудита» удалён целиком вместе с пунктом оглавления;
- разделы 6–14 перенумерованы в 5–13 (в оглавлении и в заголовках). Попутно закрылась застарелая дырка: чек-лист был §14 при отсутствующем §13;
- §10 «Данные · контракт» — удалена строка `audit_signal`; `profiles[]`, счётчики по профилю и `totals` не тронуты;
- `.ov-ausec` убран из перечня секций в §1;
- живой фрейм `f-audit` снят, а в `cockpit-desktop.js` удалены `auditCard()`/`auditRow()`, вызов в сборке кокпита и экспорт — иначе секция продолжала бы рисоваться во всех полноэкранных фреймах.

**Осталось на ваше решение — два хвоста:**
1. CSS-правила `.ov-au*` / `.ov-ausec` в `<style>` документа я **не** удалял: в задании перечислены анатомия и живые фреймы, а вы просили «ничего кроме перечисленного не менять». Сейчас это мёртвые ~20 строк. Скажите — вычищу.
2. `Overview-Cockpit-Mobile-SPEC.html` **всё ещё показывает эту секцию** — её рисует `cockpit-mobile.js` (`auditRow`, классы `.m-au-*`), а данные берутся из общего `AUDIT_SIGNALS` в `cockpit-data.js`. Правка была адресована только десктопной спеке, поэтому мобильную я не трогал. Если секцию не строим вообще — надо снять и там.

### 3. Voice: актуальная спека поднята из архива

| Файл | Было | Стало |
|---|---|---|
| `Voice-Tabs-SPEC.html` | `archive/design-export/` | `screens/role-book/` |
| `Voice-SPEC.html` | `screens/role-book/` | `archive/` |
| `Style-Rules-SPEC.html` | `screens/style-rules/` | `archive/` |

Путей в `Voice-Tabs-SPEC.html` править не потребовалось: у него только внешние ссылки (шрифты Google), локальных ассетов нет — проверено отдельно.

Обеим архивным спекам добавлена плашка сразу под заголовком: «Исторический документ. Экран перестроен 30.06.2026: «Стиль» стал вкладкой «Анти-робот» внутри «Голоса». Актуальная спецификация: Voice-Tabs-SPEC.html» — со ссылкой на новое расположение. Плашка со своими инлайновыми стилями, CSS документов не тронут.

Ссылки перенаправлены на живую спеку:
- `screens/style-rules/Style-Rules-Mobile-SPEC.html` → «Desktop spec» теперь ведёт на `../role-book/Voice-Tabs-SPEC.html`;
- `screens/account-history/Applied-Changes-History-SPEC.html` → зависимость «Voice» теперь ведёт туда же.

`Style-Rules-Mobile-SPEC.html` остался в `screens/style-rules/` — переносить мобильные версии в задании не значилось. Но по факту отдельного экрана правил стиля в продукте больше нет, так что папка описывает несуществующий маршрут. Скажите, если её тоже убрать в архив.

### Проверка после правок

82 html-файла вне `To Delete/`, **833 локальные ссылки: 0 битых, 0 ведущих в `To Delete/`.**

## Регрессия путей в JS — найдена и исправлена (26 июля)

Мой аудит ссылок был неполон и дал ложное «0 битых»: он (а) сканировал только `*.html` и ни одного `.js`, и (б) отбрасывал пути, собираемые конкатенацией строк. Из-за этого 14 файлов с путями к аватарам не проверялись вообще. На Overview-Cockpit это давало 78 битых картинок из 78.

**Причина.** Скрипты вида `const A = "assets/avatars/"` лежали в корне, где голый путь резолвился верно. После переноса документов в `screens/<slug>/` (глубина 2) и `components/` (глубина 1) префикс перестал совпадать. Важно: относительный путь внутри JS резолвится **от документа**, а не от скрипта — поэтому правильный префикс диктует глубина той страницы, которая скрипт грузит.

| Файл | Глубина потребителей | Стало |
|---|---|---|
| `screens/overview/cockpit-desktop.js` | 2 | `../../assets/avatars/` |
| `screens/overview/cockpit-mobile.js` | 2 | `../../assets/avatars/` |
| `screens/studio/studio-thread-build.js` | 2 | `../../assets/avatars/` |
| `screens/studio/studio-schedule-build.js` | 2 | `../../assets/avatars/` |
| `screens/studio/studio-ideas-build.js` | 2 | `../../assets/avatars/` |
| `screens/studio/compose-media-spec.js` | 2 | `../../assets/avatars/` |
| `screens/replies/reply-media-spec.js` | 2 | `../../assets/avatars/` |
| `screens/feed/feed-media-spec.js` | 2 | `../../assets/avatars/` |
| `screens/landing/landing-sections-demo.js` | 2 | `../../assets/avatars/` ×5 |
| `account/vk.js` | 2 | `../../assets/avatars/` |
| `components/account-empty-desktop.js` | 1 | `../assets/avatars/` |
| `components/account-empty-mobile.js` | 1 | `../assets/avatars/` |
| `account/action-card-spec.js` | 1 | `../assets/avatars/` |

**Два файла в корне грузятся с обеих глубин сразу** — `account-desktop.js` (4 документа глубины 1 + 4 глубины 2) и `account-mobile.js` (3 + 2). Для них относительного префикса, верного в обоих случаях, не существует, поэтому путь считается от самого скрипта:

```js
const A = new URL("assets/avatars/", document.currentScript.src).href;
```

Проверено в браузере: Overview-Cockpit — 78 картинок, 0 битых (было 78 из 78); Studio-Thread — 14/0; Navigation-SPEC (глубина 1, тот самый случай с двумя глубинами) — аватары из `account-desktop.js` грузятся.

### Осталось битым: `mobile/mock.js` — нужно ваше разрешение

Тот же дефект, но файл лежит в папке, которую вы запретили трогать. Строка одна:

```js
const A = 'assets/avatars/';        // mobile/mock.js, строка 10
```

`mock.js` грузят **25 документов** — 4 с глубины 1 и 21 с глубины 2, — поэтому здесь тоже нужен вариант с `currentScript`, а не префикс:

```js
const A = new URL('../assets/avatars/', document.currentScript.src).href;
```

Это регрессия от уборки (раньше все потребители лежали в корне, и голый путь был верен). Сейчас во всех мобильных спеках вместо фото — моно-заглушки. Скажите «правь» — исправлю одной строкой; сам в `mobile/` не полез, потому что вы это прямо запретили.

### Аудит переписан

Проверка теперь идёт по `.html`, `.js`, `.jsx` и `.css`, вытаскивает строковые литералы `assets/…`, `ds/…`, `mobile/…` из кода и резолвит их от глубины документа-потребителя, а не от файла. Конкатенированные пути больше не отбрасываются молча.

## Хвост секции growth-аудита — дочищено (26 июля)

Секция была снята, но её продолжала упоминать русская проза, где нет ни слова «growth», ни имён классов — три моих проверки подряд её не видели, потому что грепали по `growth`, `ov-au`, `auditRow`. Перечислено обходом отрендеренного текста, а не исходника; снято восемь мест:

| Раздел | Что стояло |
|---|---|
| §1 | порядок секций: «очередь → итоги → **строка аудита** → сетка профилей» |
| §3 | «Очередь → итоги → **строка аудита** → карточки» |
| §4 | «итоги, **строка аудита** и карточки остаются ниже» |
| §11 | брейкпоинт 760: «итоги/сетка/**строка аудита** → 1-в-ряд» |
| §11 | «итоги 4-в-ряд; **строка аудита** 3-в-ряд; карточки 2-в-ряд» |
| §11 | «итоги 2×2; **строка аудита** скроллится; карточки в столбик» |
| §13 | «итоги + **строка аудита** + карточки остаются» |
| §13 | целый критерий приёмки «**Строка аудита**: сигнал вверх/ровно/вниз + Δ + уверенность…» — чек-лист требовал принять несуществующую секцию |

**§12 «Локализация · i18n»** показывал две строки удалённой секции (`auHeading` → «Аудит роста», `proposals` → «Разобрать 2 предложения»). Они живут в массиве `I18N_LONG` в `cockpit-data.js`, который **общий с мобильной спекой**, а та секцию ещё показывает. Поэтому глобально ключи не удалял — отфильтровал их только при рендере таблицы в этом документе:

```js
var DROP = { auHeading:1, proposals:1 };
C.I18N_LONG.filter(function(r){ return !DROP[r.key]; })
```

Когда решится вопрос про мобильную версию, ключи можно будет убрать из `cockpit-data.js` и снять фильтр.

**Не тронуто намеренно:** пункт «Аудиты» в сайдбаре, строки очереди «Аудит недели готов» и тип `.ov-tr--advisor` — это живая очередь триажа, она законно ведёт в «Аудит роста».

**Проверено в браузере:** видимых упоминаний «строка аудита» — 0, «growth» — 0, элементов `.ov-au*` в DOM — 0, картинок 78 / битых 0, битых анкоров 0, консоль чистая, TOC 1–13 ↔ секции s1–s13.

## Дочистка после Pennedly-5 (26 июля)

### 1. Overview-Cockpit-SPEC.html — мёртвый CSS удалён
Блок правил снятой секции вырезан целиком (2.3 КБ): `.ov-ausec`, `.ov-au`, `.ov-au-row`, `-head`, `-av`, `-id`, `-h`, `-when`, `-sig` + `--up/--flat/--down`, `-delta`, `-lab`, `-conf`, `-cta`, `-ctatxt`, `-done`. В четырёх групповых селекторах убран только `.ov-au-av`, живые `.ov-av`, `.ov-tr-av`, `.ovsw-av`, `.sb-acct-av` сохранены. **Вхождений «ov-au» в файле: 0.**

Заодно снят временный фильтр `DROP` в рендере таблицы i18n — он больше не нужен, ключи удалены из источника (см. п. 2).

### 2. Overview-Cockpit-Mobile-SPEC.html — секция снята
- §5 «Строка growth-аудита» удалён вместе с пунктом оглавления; разделы 6–11 перенумерованы в 5–10, подзаголовок 9.1 → 8.1, дырок нет;
- `cockpit-mobile.js`: удалены `auditCard()`/`auditRow()`, оба вызова в сборке (`bodyBusy`, `bodyZero`) и экспорт — по образцу `cockpit-desktop.js`;
- CSS `.m-au-*` вырезан (2.0 КБ), из групповых селекторов убран только `.m-au-av`; живой фрейм `stg-audit` снят, строка анатомии `.m-au-row` удалена, чип `+ GrowthAuditRow` убран;
- `cockpit-data.js`: удалён массив `AUDIT_SIGNALS` и подписи, которые читала только эта секция — `auHeading`, `auSubhead`, `auReviewed`, `sigUpL/sigDownL/sigFlatL`, `confHigh/confMed/confLow/confWord/postsWord`, `propText`, плюс две строки `I18N_LONG` (`auHeading`, `proposals`) — по обеим локалям. Ключи очереди «Требует тебя» и карточек профилей (`TRIAGE*`, `triHeading`, `zeroT`, `sync.failed`, `retry` и прочие) не тронуты — проверено, таблица i18n теперь показывает ровно 7 живых ключей.

Прозаические упоминания «строка аудита» нашлись ещё в шести местах (лид, плашка «Достраивает Overview-Mobile», §1, §2, два пункта чек-листа, включая критерий приёмки) — перечислены обходом отрендеренного текста, а не грепом по исходнику, и сняты.

### 3. Feed-SPEC.html — два прозаических остатка
- лид §1: из перечня элементов карточки убрано «раскрывающийся график роста» вместе с лишней запятой;
- таблица цветов, строка `--color-accent`: убран «график роста», остальные пункты (`vbadge--settling`, `ar-pill--on`, `tb-orig`, `btn--primary`, ссылки) сохранены.

**«график роста» и «раскрывающийся» в файле: 0.**

### 4. feed.css — блок TREND PANEL удалён
Вырезаны `.trend-panel`, `.trend-cap`, `.tc-t`, `.tc-s`, `.chart-wrap` (+ `svg`), `.chart-axis` (+ `span`). Перед удалением проверено по всему корпусу: эти классы не использует ни одна живая страница — единственные вхождения были в самом `feed.css`. Обновлена шапка-комментарий файла, где панель числилась среди содержимого.

**Не тронуто намеренно:**
- `.baseline-trend` (3 вхождения) — это полоса baseline, другой компонент, живой;
- `screens/stats/First-Run-SPEC.html` — там `trend-panel` описывает график трендов Статистики, живой; `feed.css` этот документ не подключает (проверено), пересечения классов нет.

### Проверка
82 html, 833 ссылки — **0 битых, 0 в `To Delete/`**; битых якорей в трёх правленых документах — 0; консоль чистая.

В браузере: Overview-Cockpit — 0 упоминаний, `.ov-au*` в DOM 0, все живые фреймы рисуются, 78/78 картинок; Overview-Cockpit-Mobile — 0 упоминаний, `.m-au*` в DOM 0, все 8 стендов рисуются, разделы 1–10 без дырок; Feed-SPEC — 0 упоминаний.

**Осталось прежнее:** `mobile/mock.js` даёт 2 битые картинки в мобильных спеках (голый `assets/avatars/`, нужен вариант с `currentScript`) — жду вашего разрешения тронуть `mobile/`.

**Мелочь на заметку:** после снятия секции классы `.m-sec-head`/`.m-sec-ic`/`.m-sec-t` в мобильной спеке остались без потребителей (их использовал только заголовок строки аудита). В списке на удаление их не было, поэтому не трогал — скажите, если убрать.
