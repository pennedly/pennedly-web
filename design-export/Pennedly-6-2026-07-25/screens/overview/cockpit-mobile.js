...
(function () {
  ...
  function bodyBusy(lang) {
    return "<div class='m-ov'>" + triageQueue(lang) + strip(lang) + "<div class='m-ov-grid'>" + C.ORDER.map(function (id) { return card(C.PROFILES[id], lang); }).join("") + "</div>" + pad() + "</div>";
  }
  function bodyZero(lang) {
    return "<div class='m-ov'>" + triageZero(lang, 4) + strip(lang) + "<div class='m-ov-grid'>" + ["mara", "field", "studio"].map(function (id) { return card(C.PROFILES[id], lang); }).join("") + "</div>" + pad() + "</div>";
  }
  ...
  window.MOVC = {
    ic, avatar, delta,
    triageQueue, triageZero, card, strip,
